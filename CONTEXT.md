...
6. **Automatic transaction capture** (PLAN.md §7) — committed 2026-07-10 but deliberately last: Android notification listener (covers bank SMS via their notifications — never the restricted SMS permissions) → parse → user approves via prefilled deep link into `scan.tsx?manual=true`. Android-only by iOS platform limitation. Its gallery/screenshot-import spinoff is a small, cross-platform addition to the scan flow that can ship earlier
