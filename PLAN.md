...
10. **Automatic transaction capture** (see the dedicated "§7. Automatic transaction capture" section above) — **Built 2026-08-10**, Android only; **first successful EAS preview build 2026-08-11** after fixing a missing `compileSdkVersion` in the vendored module's Gradle config. Functional on-device verification (permission grant, listener actually firing, FYI notification, review deep-link) is the one remaining step before this can be considered done — currently in progress. The gallery/screenshot-import spinoff is still independent, still not built