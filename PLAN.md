...
| **App-wide press feedback** — `components/ui/AnimatedPressable.tsx`, a shared spring-physics press primitive (Reanimated, reduce-motion aware) replacing ad-hoc `TouchableOpacity`/opacity-flicker feedback everywhere (`Button`, `Card`, `IconButton`, `Chip`, etc. all route through it) | Built — 2026-08-13 |
| **Custom date/time pickers** — `components/ui/DateField.tsx` + `TimeField.tsx`, fully custom-drawn (no native OS dialog): `react-native-ui-datepicker` (pure JS, no native code) for the calendar-grid date picker, a custom brand-themed hour/minute/AM-PM stepper for time, both presented in a shared `PickerModal` bottom sheet. Replaced the native `@react-native-community/datetimepicker` (now uninstalled) after discovering OEM skins (confirmed on Samsung One UI) override native picker dialogs' colors, ignoring app theming entirely — a custom-drawn picker sidesteps that instead of fighting it | Built — 2026-08-13, on-device color verification pending |
| Finance reminder notifications — morning/evening local notifications with a curated rotating finance tip (`constants/financeTips.ts`), toggle + time pickers in Settings, `lib/financeReminders.ts` schedules two `DAILY`-trigger notifications via `expo-notifications` | Built — 2026-08-13 |
...
## 7. Automatic transaction capture (Built — 2026-08-10, Android only; EAS build passing as of 2026-08-11; native lifecycle race fixed 2026-08-13, on-device functional test still pending)

**Status: code complete, first real build succeeded, functional on-device verification still outstanding.** This session (2026-08-10) implemented the full pipeline below. Everything typechecks and the parsing/dedup/route logic is unit- and route-tested. On 2026-08-11 the first `eas build --profile preview` attempt failed at the Gradle configuration step — the vendored module's `modules/expo-notification-listener/android/build.gradle` called `useCoreDependencies()`/`useExpoPublishing()` but never `useDefaultAndroidSdkVersions()`, so AGP never got a `compileSdkVersion` and `useExpoPublishing()`'s `singleVariant("release")` failed looking for a component that didn't exist yet. Fixed by adding the missing `useDefaultAndroidSdkVersions()` call; the next EAS preview build succeeded and produced an installable APK, confirming the manifest merge and module autolinking both work in a real Gradle build.

**2026-08-13 fix — silent-capture root cause.** The user reported capture working in Expo Go but silently capturing nothing in the real preview build, even for messages that parsed correctly in isolation. Root-caused to a native lifecycle race: `NotificationListenerService` (OS-managed) can keep running and receiving notifications independently of the app process/JS module instance lifecycle, and the old code's `getInstance() ?: return` silently dropped every notification that arrived while no JS module instance existed. Fixed in `NotificationListenerModule.kt`/`NotificationListener.kt` by moving delivery state to the companion object (survives instance churn) with a bounded queue (`MAX_QUEUED = 50`) plus an explicit `markListenerReady()` JS→native handshake, since native `EventEmitter`s don't buffer events emitted before a JS listener subscribes — a second, previously-unhandled race. Not yet confirmed on a real device.

**Still open**: the actual functional behavior on a device — notification-listener permission grant, `onNotificationPosted` callbacks actually firing, the FYI notification, and tap-to-inbox — has not been confirmed working yet, only that the build compiles and installs.
...
- Functional on-device verification: grant notification access via the Settings deep link, trigger a real notification from an allowlisted app, confirm it lands in `app/captured.tsx`, confirm the FYI notification fires and tap-to-inbox works, confirm **Review** deep-links into `scan.tsx` prefilled correctly, and confirm the 2026-08-13 queueing/handshake fix actually resolves the silent-capture bug — none of this is confirmed yet, only that the build compiles and installs
...
- ~~AI data-sharing disclosure (§6, still "Planned")~~ — **Built 2026-08-13**, see §6 above; covers the Gemini notification-parsing fallback alongside receipt OCR, chat quick-add, and recaps

**2026-08-13 — related release-build-only bug fixed (reminders, not capture):** finance reminder notifications (§3) worked in Expo Go but failed in the real preview build with "Couldn't update reminders" — root-caused to `expo-notifications` not wiring its own `proguard-rules.pro` as `consumerProguardFiles`, so R8 minification (enabled via `app.json`) could strip/rename its internal scheduling classes. Fixed via `expo-build-properties`'s `extraProguardRules` in `app.json`. Also fixed, same investigation: this project is Expo **managed workflow** (`android`/`ios` are gitignored, regenerated fresh by `expo prebuild` on every EAS build), so earlier hand-edits to native files had been silently discarded every build — all native customization now goes through `app.json`/config plugins (`plugins/withAndroidBrandTheme.js`). Not yet confirmed by a real build.

**Cheap spinoffs that can ship earlier, independently:**
| Addition | Notes |
|---|---|
| Gallery/screenshot import in scan flow | `launchImageLibraryAsync` alongside the existing camera source — payment-confirmation screenshots go through the same Gemini Vision pipeline | **Built — 2026-08-13**: `components/ui/ImageSourceModal.tsx` (source-picker bottom sheet — "Take Photo" / "Choose from Gallery"), `app/scan.tsx`'s capture logic split into a shared `processPickedImage()` so both sources feed the same OCR pipeline |
| Android share target | Share a receipt screen/PDF from another app straight into the scan pipeline; intent-filter addition | Planned |
...
10. **Automatic transaction capture** (see the dedicated "§7. Automatic transaction capture" section above) — **Built 2026-08-10**, Android only; **first successful EAS preview build 2026-08-11** after fixing a missing `compileSdkVersion` in the vendored module's Gradle config; **native lifecycle race (silent-capture bug) fixed 2026-08-13**. Functional on-device verification (permission grant, listener actually firing, FYI notification, review deep-link, and the 2026-08-13 fix itself) is the one remaining step before this can be considered done — currently in progress. The gallery/screenshot-import spinoff is **built — 2026-08-13** (`ImageSourceModal`, works on both Android and iOS since it's plain `expo-image-picker`)